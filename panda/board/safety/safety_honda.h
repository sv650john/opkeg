...
const safety_hooks honda_nidec_hooks = {
  ...
  .fwd = default_fwd_hook, //Clarity: Normally OpenPilot forwards messages between the car and the fatory ADAS Camera, but since we don't have the facotry camera connected, we must not forward any messages or else we bridge the two F-CANs. -wirelessnet2
